...
final class Provisioning {
    public static function install(string $clientId, string $writeKey, string $readKey): void {
        ...
        fwrite($pipes[0], json_encode(['client_id' => $clientId, 'write_key' => $writeKey, 'read_key' => $readKey]));
        fclose($pipes[0]);
